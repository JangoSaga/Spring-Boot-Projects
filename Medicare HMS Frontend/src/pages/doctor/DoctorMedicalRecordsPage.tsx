...
import { Eye, FileText, X, User } from 'lucide-react';
...
export const DoctorMedicalRecordsPage: React.FC = () => {
  ...
  const { data: records, isLoading: recordsLoading, error, refetch } = useQuery({
    queryKey: ['doctorMedicalRecords', patients?.length, user?.id],
    queryFn: async () => {
      ...
      // Filter strictly to records written by this doctor
      return all.filter(r => r.doctorId === user!.id);
    },
    enabled: !!patients && !!user?.id,
  });
  ...
  const columns = [
    ...
    { header: 'Diagnosis Logged', accessor: 'diagnosis' as const, className: 'font-semibold text-slate-700' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between bg-white p-5 rounded-xl border border-slate-200 shadow-sm shrink-0">
        <div>
          <h1 className="text-xl font-bold text-slate-800">My Consultation Archive</h1>
          <p className="text-xs text-slate-500 mt-0.5">Dossier of clinical diagnostic reports, prescriptions, and assessment records you filed.</p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-2xl max-w-lg w-full p-6 relative z-10 animate-fade-in flex flex-col gap-4">
            ...
            <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
              ...
              <h3 className="text-base font-bold text-slate-800">Assessed Consultation Record</h3>
            </div>
            ...
            <div className="space-y-4 text-xs">
              ...
              <div>
                <span className="block text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">Assessed Diagnosis</span>
              ...
              <div>
                <span className="block text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">Filed Prescriptions</span>
              ...
              <div>
                <span className="block text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">Treatment Assessment Notes</span>
